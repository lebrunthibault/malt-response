...
1. `get` — authedProcedure query:
   - Queries `profiles` table: `supabase.from('profiles').select('display_name, email, created_at').eq('id', ctx.user.id).single()`
   - Returns `{ displayName: string | null, email: string, createdAt: string }`
